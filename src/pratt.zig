...
const trasform = @import("transform.zig");

const PratNode = trasform.PratNode;

pub const PrattError = error {
    OutOfMemory,
...
pub const ExpressionParser = struct {
    ...
    fn consumeNullDescriptor(self: *Self) !astnode.ASTExpression {
                ...
                .LBrace => {
                    ...
                    while (true) {
                        const node = self.peekToken()
                            orelse return error.InvalidEndOfStream;
                        switch (node) {
                            .Token => |toke| {
                                if (toke.token == .Comma) {
                                    self.advance();
    ...
    pub fn pratt(self: *Self, rbp: u32) PrattError!astnode.ASTExpression {
    ...
    fn peekOperator(self: *const Self) !OperatorInfo {
        // Give me a token
    ...
    fn peekToken(self: *const Self) ?PratNode {
